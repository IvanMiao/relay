...
export async function createOpenAIAgentsRuntime() {
  if (!process.env.OPENAI_API_KEY?.trim()) {
    throw new Error("Set OPENAI_API_KEY in your server environment or .env.local.");
  }
  const { default: OpenAI } = await import("openai");
  // Automatic POST retries could duplicate input after an ambiguous response.
  return new OpenAIAgentsRuntime(new OpenAI({ maxRetries: 0, timeout: 30_000 }));
}

export class OpenAIAgentsRuntime {
  constructor(client) {
    if (!client.beta?.agents?.sessions) {
      throw new Error("Installed OpenAI SDK does not expose beta.agents.sessions. Update openai.");
    }
  ...
  async savedItems(sessionId) {
    ...
    for await (const item of this.sessions.items.list(sessionId, { order: "asc", limit: 100 })) {
      items.push(item);
    }
    return items;
  }
    ...
    try {
      for await (const event of stream) {
        ...
        if (event.type === "agent.session.requires_action") {
          if (!sessionId) throw new Error("Required action arrived without a session ID.");
          ...
          for (const action of session.required_actions) {
            ...
            if (!outcome || (outcome.success === true ? typeof outcome.output !== "string"
              : outcome.success !== false || typeof outcome.error !== "string")) {
              throw new Error("Tool dispatcher must return a serialized success or error result.");
            }
            await this.sessions.events.create(sessionId, { events: [{
              type: "agent.session.input.tool_result",
              turn_id: action.turn_id, call_id: action.call_id,
              ...(outcome.success ? { success: true, output: outcome.output }
        ...
        if (event.turn?.subagent_id === null) {
          ...
          if (event.type === "agent.session.turn.completed") {
            if (!sessionId || !event.turn.id) throw new Error("Completed turn lacks identity.");
            return { sessionId, turnId: event.turn.id, outcome: "turn_completed" };
          }
        }
      }
      throw new Error("Stream ended without a completed turn. Retrieve session and saved items before retrying.");
    } finally {
      stream.controller.abort();
    }
  }
