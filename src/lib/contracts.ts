...
export type CaseStage = "discover" | "prepare" | "review" | "execute" | "verify";
export type CaseStatus =
  | "running" | "waiting_for_reply" | "waiting_for_authorization"
  | "paused" | "failed" | "completed";
export type EvidenceStatus = "confirmed" | "inferred" | "conflicting" | "unknown" | "superseded";
export type TaskStatus =
  | "ready" | "in_progress" | "waiting_for_information"
  | "waiting_for_authorization" | "completed" | "failed" | "canceled";
...
export interface CaseFacts {
  ...
  quote: ArtifactRef;
  missingFields: Array<keyof DraftFields | "businessPurpose" | "entity" | "requestedByDate">;
}
...
export interface Clarification {
  ...
  question: string;
  evidenceIds: string[];
  deliveryStatus: "pending_authorization" | "pending_delivery" | "delivered" | "failed";
...
export interface CaseEvent {
  ...
  kind: "case_created" | "evidence_updated" | "clarification_updated"
    | "action_proposed" | "authorization_recorded" | "execution_updated"
    | "control_updated" | "draft_verified" | "error";
...
export interface CaseSnapshot {
  ...
  blocker: { code: "missing_information" | "authorization_required" | "evidence_conflict"
    | "execution_error" | "reconciliation_required"; message: string; taskId: string | null } | null;
  people: PersonRole[];
  tasks: CaseTask[];
  evidence: Evidence[];
...
export interface CaseResponse { case: CaseSnapshot }
export type ApiErrorCode = "invalid_request" | "not_found" | "forbidden"
  | "stale_version" | "idempotency_conflict" | "invalid_state"
  | "action_expired" | "validation_failed" | "internal_error";
...
export interface PortalDraftResponse { draft: PortalDraft }
export interface PortalDraftLookupResponse { draft: PortalDraft | null }
...
export interface RelayEndpoints {
  ...
  "POST /api/cases/:id/replies": { request: ReplyRequest; response: CaseResponse };
  "POST /api/cases/:id/authorizations": { request: AuthorizationRequest; response: CaseResponse };
  "POST /api/cases/:id/control": { request: ControlRequest; response: CaseResponse };
  "GET /api/artifacts/:id": { response: "binary" };
  "POST /api/portal/drafts": { request: CreatePortalDraftPayload; response: PortalDraftResponse };
  "GET /api/portal/drafts/:id": { response: PortalDraftResponse };
  "GET /api/portal/drafts?requestReference=:reference": { response: PortalDraftLookupResponse };
}
