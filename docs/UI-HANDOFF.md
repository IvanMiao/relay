...
src/lib/contracts.ts is the initial shared snapshot contract. The teammate maintains it going forward, coordinating changes with the UI. Initial examples are in src/components/relay/preview.ts; these fixtures intentionally stay in the UI directory instead of occupying the teammate's fixtures/ directory.

The adapter in src/components/relay/use-relay-case.ts calls:

| Operation | Request |
| --- | --- |
| Create case | POST /api/cases with request, optional quoteArtifactId, and sourceUrl |
...
| Authorize or decline | POST /api/cases/:id/authorizations with actionId, expectedVersion, decision: allow/decline |
| Pause or resume | POST /api/cases/:id/control with eventId, expectedVersion, action: pause/resume |

Creation must return JSON containing id or case.id. Reads accept the snapshot directly or under case or snapshot. Mutations may return a snapshot or a JSON acknowledgment; an acknowledgment triggers a fresh read. Failed responses should include an error string. Stale action reviews should return 409.
...
To display the real quote, supply facts.quoteArtifactId and implement GET /api/artifacts/:id. New-request artifact upload is not implemented in this UI track; an existing case can supply an artifact ID, and the agent fixtures can register the initial quote. Do not treat the visible preview quote as an artifact uploaded to the agent.

## Procurement destination contract

POST /api/portal/drafts accepts **multipart form data**:

| Field | Rule |
| --- | --- |
| requestReference | Required; 1–80 characters; letters, numbers, dashes, underscores; starts with a letter or number |
| item | 2–200 characters |
| vendor | 2–160 characters |
| quantity | Integer from 1 to 10,000 |
| currency | USD, EUR, or GBP |
| unitPrice | Positive, at most 1,000,000, at most two decimal places |
...
The matching browser labels are **Item**, **Vendor**, **Request reference**, **Quantity**, **Currency**, **Unit price**, **Cost center**, **Business justification**, and **Save draft**. The file input has id="attachments" and name="attachments".

A successful create returns { draft, reused: false } with status 201. An exact retry returns the same draft with reused: true and status 200. A different payload using an existing reference returns 409. Validation returns 422 with error and optional field errors.

Reusing a reference requires the same normalized fields, attachment bytes, names, MIME types, and order. Preserve the reference across uncertain retries. Use a new reference for a separate intended draft. The reference is unique in SQLite, and draft plus attachment writes commit in one transaction.

| Read endpoint | Response |
| --- | --- |
| GET /api/portal/drafts | { drafts }, newest first, up to 100 |
| GET /api/portal/drafts?requestReference=... | { draft } or 404 |
| GET /api/portal/drafts/:id | { draft } or 404 |
| GET /api/portal/attachments/:id | Original file bytes as a download, or 404 |

Draft records contain id, requestReference, status: draft, createdAt, fields, attachments, and a relative url. Each attachment includes its name, content type, byte size, SHA-256, and download URL. The portal does not manufacture an agent verification timestamp.
